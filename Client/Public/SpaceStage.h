...
class CSpaceStage final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CSpaceStage(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CSpaceStage(const CSpaceStage& Prototype);
	virtual ~CSpaceStage() = default;
...
public:
	static CSpaceStage* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
