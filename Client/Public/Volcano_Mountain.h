...
class CVolcano_Mountain final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CVolcano_Mountain(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CVolcano_Mountain(const CVolcano_Mountain& Prototype);
	virtual ~CVolcano_Mountain() = default;
...
public:
	static CVolcano_Mountain* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
