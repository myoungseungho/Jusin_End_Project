...
class CVolcano_SkyCloud final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CVolcano_SkyCloud(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CVolcano_SkyCloud(const CVolcano_SkyCloud& Prototype);
	virtual ~CVolcano_SkyCloud() = default;
...
public:
	static CVolcano_SkyCloud* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
