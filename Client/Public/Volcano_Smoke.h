...
class CVolcano_Smoke final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CVolcano_Smoke(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CVolcano_Smoke(const CVolcano_Smoke& Prototype);
	virtual ~CVolcano_Smoke() = default;
...
public:
	static CVolcano_Smoke* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
