...
class CSpaceEarth_Light final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CSpaceEarth_Light(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CSpaceEarth_Light(const CSpaceEarth_Light& Prototype);
	virtual ~CSpaceEarth_Light() = default;
...
public:
	static CSpaceEarth_Light* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
