...
class CVolcano_GroundRock final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CVolcano_GroundRock(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CVolcano_GroundRock(const CVolcano_GroundRock& Prototype);
	virtual ~CVolcano_GroundRock() = default;
...
public:
	static CVolcano_GroundRock* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
