...
class CMelee_Effect_2p final : public CGameObject
{
public:
	enum class PLAYERID
	{
...
private:
	CMelee_Effect_2p(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CMelee_Effect_2p(const CMelee_Effect_2p& Prototype);
	virtual ~CMelee_Effect_2p() = default;
...
public:
	static CMelee_Effect_2p* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
