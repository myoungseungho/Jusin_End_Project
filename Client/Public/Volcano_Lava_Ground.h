...
class CVolcano_Lava_Ground final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CVolcano_Lava_Ground(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CVolcano_Lava_Ground(const CVolcano_Lava_Ground& Prototype);
	virtual ~CVolcano_Lava_Ground() = default;
...
public:
	static CVolcano_Lava_Ground* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
