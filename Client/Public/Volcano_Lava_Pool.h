...
class CVolcano_Lava_Pool final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CVolcano_Lava_Pool(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CVolcano_Lava_Pool(const CVolcano_Lava_Pool& Prototype);
	virtual ~CVolcano_Lava_Pool() = default;
...
public:
	static CVolcano_Lava_Pool* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
