...
class CSpaceCliff final : public CGameObject
{
public:
	enum Space_TextureType { SPACE_DIFFUSE,SPACE_STAR_1, SPACE_STAR_2, SPACE_END};

private:
	CSpaceCliff(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CSpaceCliff(const CSpaceCliff& Prototype);
	virtual ~CSpaceCliff() = default;
...
public:
	static CSpaceCliff* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
