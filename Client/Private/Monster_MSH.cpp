...
CMonster_MSH::CMonster_MSH(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject{ pDevice, pContext }
{

}

CMonster_MSH::CMonster_MSH(const CMonster_MSH& Prototype)
	: CGameObject{ Prototype }
{

}

HRESULT CMonster_MSH::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMonster_MSH::Initialize(void* pArg)
{
...
void CMonster_MSH::Priority_Update(_float fTimeDelta)
{

}

void CMonster_MSH::Update(_float fTimeDelta)
{
	...
	{
		CPlayer_MSH* mainPlayer = static_cast<CPlayer_MSH*>(m_pGameInstance->Get_GameObject(LEVEL_GAMEPLAY, TEXT("Layer_Player")));
...
HRESULT CMonster_MSH::Render(_float fTimeDelta)
{
...
void CMonster_MSH::OnCollisionEnter(CCollider* other, _float fTimeDelta)
{
	int a = 3;
}

void CMonster_MSH::OnCollisionStay(CCollider* other, _float fTimeDelta)
{
	int a = 3;
}

void CMonster_MSH::OnCollisionExit(CCollider* other)
{
	int a = 3;
}

HRESULT CMonster_MSH::Ready_Components()
{
	/* Com_Shader */
...
HRESULT CMonster_MSH::Bind_ShaderResources()
{
...
CMonster_MSH* CMonster_MSH::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CMonster_MSH* pInstance = new CMonster_MSH(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CMonster_MSH"));
...
CGameObject* CMonster_MSH::Clone(void* pArg)
{
	CMonster_MSH* pInstance = new CMonster_MSH(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CMonster_MSH"));
		Safe_Release(pInstance);
	}
