#include "stdafx.h"
#include "..\Public\Energy_Effect_2p.h"
...
CEnergy_Effect_2p::CEnergy_Effect_2p(const CEnergy_Effect_2p& Prototype)
	: CGameObject{ Prototype }
{

}

HRESULT CEnergy_Effect_2p::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CEnergy_Effect_2p::Initialize(void* pArg)
{
	if (FAILED(__super::Initialize(pArg)))
		return E_FAIL;

	m_playerID = CEnergy_Effect_2p::PLAYERID::PLAYER_2P;

	return S_OK;
}

void CEnergy_Effect_2p::Priority_Update(_float fTimeDelta)
{

}

void CEnergy_Effect_2p::Update(_float fTimeDelta)
{
	...
	switch (m_playerID)
	{
	case CEnergy_Effect_2p::PLAYERID::PLAYER_1P:
		group = CCollider_Manager::CG_1P_Energy_Attack;
		break;
	case CEnergy_Effect_2p::PLAYERID::PLAYER_2P:
...
void CEnergy_Effect_2p::Late_Update(_float fTimeDelta)
{
...
HRESULT CEnergy_Effect_2p::Render(_float fTimeDelta)
{
	return S_OK;
}

void CEnergy_Effect_2p::OnCollisionEnter(CCollider* other, _float fTimeDelta)
{
	switch (m_playerID)
	{
		//이펙트가 1P_Skill이라면
...
void CEnergy_Effect_2p::OnCollisionStay(CCollider* other, _float fTimeDelta)
{
	int a = 3;
}

void CEnergy_Effect_2p::OnCollisionExit(CCollider* other)
{
	int a = 3;
}

void CEnergy_Effect_2p::Make_Collider(CCollider_Manager::COLLIDERGROUP eColliderGroup, _float2 SourcePos, _float2 DestPos)
{
...
CEnergy_Effect_2p* CEnergy_Effect_2p::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEnergy_Effect_2p* pInstance = new CEnergy_Effect_2p(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CEnergy_Effect_2p"));
...
CGameObject* CEnergy_Effect_2p::Clone(void* pArg)
{
	CEnergy_Effect_2p* pInstance = new CEnergy_Effect_2p(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CEnergy_Effect_2p"));
		Safe_Release(pInstance);
	}
