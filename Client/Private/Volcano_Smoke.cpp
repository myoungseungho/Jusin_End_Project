...
CVolcano_Smoke::CVolcano_Smoke(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject { pDevice, pContext }
{

}

CVolcano_Smoke::CVolcano_Smoke(const CVolcano_Smoke & Prototype)
	: CGameObject{ Prototype }
{

}

HRESULT CVolcano_Smoke::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CVolcano_Smoke::Initialize(void * pArg)
{
...
HRESULT CVolcano_Smoke::Ready_Components()
{
	/* Com_Shader */
...
HRESULT CVolcano_Smoke::Bind_ShaderResources()
{
...
CVolcano_Smoke * CVolcano_Smoke::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CVolcano_Smoke*		pInstance = new CVolcano_Smoke(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CVolcano_Smoke"));
...
CGameObject * CVolcano_Smoke::Clone(void * pArg)
{
	CVolcano_Smoke*		pInstance = new CVolcano_Smoke(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CVolcano_Smoke"));
		Safe_Release(pInstance);
	}
